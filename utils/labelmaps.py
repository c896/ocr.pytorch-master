...
def get_vocabulary(voc_type, EOS='EOS', PADDING='PADDING', UNKNOWN='UNKNOWN'):
  ...
  if voc_type == 'LOWERCASE':
    voc = list(string.digits + string.ascii_lowercase)#数字+小写字母
  elif voc_type == 'ALLCASES':
    voc = list(string.digits + string.ascii_letters)#数字+大小写字母不包含符号
  elif voc_type == 'ALLCASES_SYMBOLS':
    voc = list(string.printable[:-6])#数字+大小写字母包含字符
  else:
